...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        self.is_sim = True
        if 'v2v4real' in params['root_dir']:
            self.is_sim = False
    ...
    def reform_param(self, cav_content, ego_content, timestamp_cur,
        ...
        if not self.is_sim:
            filtered_vehicles = {}
            for id, content in cur_params['vehicles'].items():
                if content['obj_type'].lower() == 'car':
                    filtered_vehicles[id] = content

            delay_params['vehicles'] = filtered_vehicles
        else:
