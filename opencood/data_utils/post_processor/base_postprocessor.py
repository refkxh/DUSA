...
class BasePostprocessor(object):
    ...
    def generate_object_center_dairv2x(self,
        ...
        object_list_coop = cav_content['params']['vehicles_coop'] # world coord.
        filter_range = self.params['anchor_args']['cav_lidar_range']

        output_dict = {}
        box_utils.project_world_objects_dairv2x(object_list_coop,
                                                output_dict,
                                                reference_lidar_pose,
                                                filter_range,
                                                self.params['order'])
        ...
        if return_visible_mask:
            object_list_single = cav_content['params']['vehicles']
    ...
    def generate_object_center_dairv2x_late_fusion(self, cav_contents):
        ...
        cav_content = cav_contents[0]
        if 'vehicles' in cav_content['params']:
            tmp_object_list = cav_content['params']['vehicles']
        else:
            tmp_object_list = cav_content['params']['vehicles_coop'] # ego coord.
