...
from opencood.visualization.simple_plot3d import canvas_3d, canvas_bev
...
def visualize_single_sample_output_gt(pred_tensor,
    ...
    pred_score_tensor : torch.Tensor
        (N,) prediction score.
    ...
    pc_range = [int(i) for i in dataset.params["preprocess"]["cav_lidar_range"]]

    if len(pcd.shape) == 3:
        pcd = pcd[0]
    pcd_np = pcd
    if not isinstance(pcd_np, np.ndarray):
        pcd_np = common_utils.torch_tensor_to_numpy(pcd_np)

    if pred_tensor is not None:
        pred_np = pred_tensor.cpu().numpy()
        pred_score_np = pred_score_tensor.cpu().numpy()
        pred_name = [f'{pred_score_np[i]:.2f}' for i in range(pred_score_np.shape[0])]

    gt_np = gt_tensor.cpu().numpy()
    gt_name = [''] * gt_np.shape[0]

    left_hand = False if 'DAIR' in dataset.params["fusion"]["core_method"] else True

    save_dir = os.path.dirname(save_path)
    save_filename = os.path.basename(save_path)
    save_dir_bev = os.path.join(save_dir, 'bev')
    os.makedirs(save_dir_bev, exist_ok=True)
    save_path_bev = os.path.join(save_dir_bev, save_filename)
    save_dir_3d = os.path.join(save_dir, '3d')
    os.makedirs(save_dir_3d, exist_ok=True)
    save_path_3d = os.path.join(save_dir_3d, save_filename)
    ...
    box_line_thickness_bev = 2
    canvas.draw_boxes(gt_np, colors=(0, 255, 0), texts=gt_name, box_line_thickness=box_line_thickness_bev)
    if pred_tensor is not None:
        canvas.draw_boxes(pred_np, colors=(255, 0, 0), texts=pred_name, box_line_thickness=box_line_thickness_bev)

    plt.figure(figsize=(5.1, 1.9))
    plt.axis("off")
    plt.imshow(canvas.canvas)
    plt.tight_layout(pad=0.0)
    plt.savefig(save_path_bev, transparent=False, dpi=400, pad_inches=0.0)
    plt.close()
    ...
    box_line_thickness_3d = 2
    canvas.draw_boxes(gt_np, colors=(0, 255, 0), texts=gt_name, box_line_thickness=box_line_thickness_3d)
    if pred_tensor is not None:
        canvas.draw_boxes(pred_np, colors=(255, 0, 0), texts=pred_name, box_line_thickness=box_line_thickness_3d)

    plt.figure(figsize=(2.5, 1.25))
    plt.axis("off")
    plt.imshow(canvas.canvas)
    plt.tight_layout(pad=0.0)
    plt.savefig(save_path_3d, transparent=False, dpi=400, pad_inches=0.0)
    plt.close()
...
def visualize_single_sample_output_bev(pred_box, gt_box, pcd, dataset,
    ...
    if 'res' in dataset.params["preprocess"]["args"]:
