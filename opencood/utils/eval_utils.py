...
def caluclate_tp_fp(det_boxes, det_score, gt_boxes, result_stat, iou_thresh):
    ...
    if det_boxes is not None:
        ...
        score = det_score[score_order_descend].tolist()  # from high to low
...
def calculate_ap(result_stat, iou):
    ...
    assert len(fp) == len(tp) == len(score)

    # sort the prediction bounding box by score
    score_order_descend = np.argsort(-score)
    fp = fp[score_order_descend].tolist()  # from high to low
    tp = tp[score_order_descend].tolist()  # from high to low
